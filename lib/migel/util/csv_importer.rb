  ...
  module Util
    class CsvImporter
      ...
      def report(lang = "de")
        ...
        if @migel_codes_without_products && (@migel_codes_without_products.size > 0)
        ...
        CSV.foreach(file_name, col_sep: ";") do |line|
          ...
          if migelid = Migel::Model::Migelid.find_by_migel_code(migel_code)
            ...
            if with_matching_pharmacode.size == 1
              ...
              puts "#{Time.now}: Skipping : " + estimate_time(@start_time, total, count, " ") + "migel_code: #{migel_code} #{ean13}/#{pharmacode}"
            end
          else
      ...
      def delete_all_inactive_bauerfeind_products
        ...
        Migel::Model::Migelid.all.each do |migel_id|
          ...
          migel_id.products.find_all { |x| x&.companyname && x.companyname.de && /bauerfeind/i.match(x.companyname.de.force_encoding("UTF-8")) }.each do |product|
          ...
          migel_id.products.delete_if { |x| x&.companyname && x.companyname.de && /bauerfeind/i.match(x.companyname.de.force_encoding("UTF-8")) }
      ...
      def clear_inactive_bauerfeind_products
        ...
        Migel::Model::Migelid.all.each do |migel_id|
          migel_code = migel_id.migel_code
          nr_invalids = migel_id.products.count { |x| x.status == "I" && /Bauerfeind/i.match(x.companyname.to_s) }
          if nr_invalids > 0
            items += nr_invalids
      ...
      def set_bauerfeind_products_inactive
        items = 0
        Migel::Model::Product.all.find_all { |x| x.status == "A" && /bauerfeind/i.match(x.companyname.to_s) }.each { |x|
          x.status = "I"
          x.save
          items += 1
        }
